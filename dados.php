...
function ligar_bco(&$err) {
     try {
          ...
          if ( $_SESSION['wrktipban'] == "mysql" ) { 
               if ($ip == "127.0.0.1") {
                    $dsn = 'mysql:host=localhost; dbname=bd_nielsen'; 
                    $usu = 'root';
                    $pas = '';
               } else {
                    $dsn = 'mysql:host=localhost; dbname=profsa92_bd_nielsen'; // Acesso site Profsa
                    $usu = 'profsa92_profsa';
                    $pas = 'pallas41';     
               }
          ...
          if ( $_SESSION['wrktipban'] == "access" ) { 
               ...
               $pdo = new PDO($str) or die('Erro na conexão PDO (Access) com o banco de dados bd_nielsen');                 
          }   
          if ( $_SESSION['wrktipban'] == "postgre" ) { 
               $dsn = 'pgsql:host=localhost; port=5432;dbname=bd_nielsen'; 
               $usu = 'nielsen';
               $pas = 'profsa1993';
               $pdo = new PDO($dsn, $usu, $pas) or die('Erro na conexão PDO (PostGre) com o banco de dados bd_nielsen');       
          }   
          return $pdo;
     }
